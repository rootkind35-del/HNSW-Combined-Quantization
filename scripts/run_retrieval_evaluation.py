...
def main():
    ...
    print("\n" + "="*80)
    print(f"{'THUẬT TOÁN':<28} | {'QPS':<10} | {'MEAN LATENCY (ms)':<20} | {'P95 LATENCY (ms)':<20} | {'RECALL@' + str(args.top_k):<10}")
    print("-" * 80)
    for algo in ['hnsw', 'two_tier', 'cf_distributed']:
        data = report[algo]
        if algo == 'hnsw':
            algo_name = "Standard HNSW"
        elif algo == 'two_tier':
            algo_name = "Two-Tier Quantized HNSW"
        else:
            algo_name = "Distributed CF"
        print(f"{algo_name:<28} | {data['qps']:<10.2f} | {data['latency_mean_ms']:<20.2f} | {data['latency_p95_ms']:<20.2f} | {data['recall_at_k']:<10.4f}")
    print("="*80)
